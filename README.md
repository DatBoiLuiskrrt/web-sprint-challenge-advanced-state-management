...
1. What problem does the context API help solve?
   Context api solves the problems of prop drilling. We no longer have to pass data down from parent to child and thus allowing other components have access to the data even when they might not need it. Context api, is a way to share state between unrelated or distant components. What we do is we wrap our components in a context.provider and then call usecontext(context) inside that component to access our state.
2. In your own words, describe `actions`, `reducers` and the `store` and their role in Redux. What does each piece do? Why is the store known as a 'single source of truth' in a redux application?
   The role of actions and action creators is the bridge that connects to the reducers, and tells the reducers what should be updated in the store. The store is a immutable state, where all changes are handled by the actions. The reducers make it so that you have clear separation of concerns, there is no prop drilling and they provide immutable data. Reducers take two values and return one. The take the current state object and an action object. Reducers are what determine how our data is manipulated.

3. What does `redux-thunk` allow us to do? How does it change our `action-creators`?
   What redux thunk does is it gives redux the ability to run async operation. "thunk" it's just a term for a function that is returning a function.
4. What is your favorite state management system you've learned and this sprint? Please explain why!
   I feel like it really just depends on the size of the project. I like the simplicity of useState but I hate it when I start having to pro drill through a lot of components, it drives me crazy debugging it. I feel like redux is just way too complicated, there is just so many extra steps that are needed to even make an api call. Then we have context api, which to me seems like a lot simpler option that keeps it clean but short. So, if I had to choose I guess I would prefer to use ContextAPI.
